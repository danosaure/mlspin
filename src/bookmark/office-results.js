...
(() => {
    ...
    const agentMap = (officeId, officeName, streetAddress, city, state, zip) => (i, tr) => [
        $(SELECTORS.AGENT_CONTACT, tr).parent().attr('href').replace(/^.*agentId=/, '').replace(/&.*/, ''),
    ...
    allRows.map((i, tr) => {
        ...
        iframe.load(() => {
            ...
            const content = $(SELECTORS.AGENT_ROWS, iframeContents).map(agentMap(officeId, officeName, streetAddress, city, state, zip)).get();
            if (content.length) {
